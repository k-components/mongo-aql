...
describe('Built-In Query Types', function(){
	...
	describe('Type: aql', function(){
		...
		it ('should build a query { "_key": { "$in": [ "abc", "def" ] } }', function(){
			...
			expect(query.query).eql('FOR c IN a-table FILTER c.@v0 in [@v1, @v2] RETURN c');
			expect(query.values).eql({ v0: '_key', v1: 'abc', v2: 'def' });
		});
		...
		it ('should build a query { "_key": { "$in": [ "abc", "def" ] }, "_type": { "$ne": null } }', function(){
			...
			expect(query.query).eql('FOR c IN a-table FILTER c.@v0 in [@v1, @v2] && c.@v3 != null RETURN c');
			expect(query.values).eql({ v0: '_key', v1: 'abc', v2: 'def', v3: '_type' });
		});
