    ...
    for (var key in where) {
      ...
      if (typeof where[key] == 'object' && !(where[key] instanceof Date) && !Buffer.isBuffer(where[key])) {
        ...
        // Key is column
          ...
          (result = buildConditions(where[key], condition, key, ' && ', fields.concat(key))) && conditions.push(result);
        }
