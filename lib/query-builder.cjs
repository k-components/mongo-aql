...
var queryTypes = require('./query-types.cjs');
var queryHelpers = require('./query-helpers.cjs');
var utils = require('./utils.cjs');

module.exports = function(collection, json, alias, valuePrefix) {
	var query;
	alias = alias || 'c';
	valuePrefix = valuePrefix || 'v';
	global.valuePrefix = valuePrefix;
...
function build(query, values) {
	...
	var type = queryTypes.get(query.type),
	...
	for (var i = 0, l = variables.length, key; i < l; ++i){
		...
		type = type.replace(
			variables[i],
			queryHelpers.has(key = variables[i].substring(1, variables[i].length - 1)) && query[key] ? queryHelpers.get(key).fn(query[key], values, query) : ''
		);
	}
