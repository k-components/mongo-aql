...
module.exports.neighbors = function(graphName, vertexExample, edgeExample, options) {
	...
	if (edgeExample) {
		var query = 'FOR v, e, p in ' + (direction[options.direction] || direction.any) + ' @vertexExample GRAPH @graphName ',
		...
		var query = 'FOR v, e, p in ' + (direction[options.direction] || direction.any) + ' @vertexExample GRAPH @graphName prune condition = v == null filter !condition ',
			values = { graphName: graphName, vertexExample: vertexExample };
