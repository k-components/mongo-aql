...
module.exports.edge = function(graphName, from, to, edgeExample, options) {
	...
	var values = { graphName: graphName, from: from },
		query = 'FOR v, e, p in ' + (direction[options.direction] || direction.outbound) + ' @from GRAPH @graphName OPTIONS { bfs: true } FILTER p.vertices[1]._id == ' + utils.newVar(to, values);
	...
	query += ' RETURN { d: v._key, v: 1, type: "http://sharejs.org/types/JSONv0", data: e }';
...
module.exports.neighbors = function(graphName, vertexExample, edgeExample, options) {
	...
	if (edgeExample) {
		var query = 'FOR v, e, p in ' + (direction[options.direction] || direction.any) + ' @vertexExample GRAPH @graphName OPTIONS { bfs: true } FILTER p.vertices[1] != null AND ',
		...
		var query = 'FOR v, e, p in ' + (direction[options.direction] || direction.any) + ' @vertexExample GRAPH @graphName OPTIONS { bfs: true } FILTER p.vertices[1] != null ',
			values = { graphName: graphName, vertexExample: vertexExample };
